...
([`oh-my-dsh/dsh-plugin-upgrade-skill`](https://github.com/oh-my-dsh/dsh-plugin-upgrade-skill)) as a
submodule pinned to one commit — never to `main`, because their own comparability rules require a frozen
snapshot. Their tasks ship reference answers, and an oracle run must score exactly `1.0`; that self-check
runs here without Harbor or an API key:
...
The second form scores **this project's own agent** on their exam tasks: `tools/harbor/dsh_agent.py` is a
Harbor agent adapter that uploads the shipped migrate profile into the task container and runs the task
statement verbatim. Measured locally: `M1-host-migration` **1.000**, `H1-plane-trap` **1.000**,
`S1-static-scan` 0.000 (an `AgentTimeoutError` against that task's 300s agent budget).

The check also runs a second arm that differs by exactly one line — this Action's `DSH_HOME` — because
their judge hardcodes `/root/.dsh/profiles`: measured locally, the task environment scores `1.0` and the
same environment plus `DSH_HOME` scores `0.4`.

Records of what each run produced live in [reports/](reports/README.md), in a versioned format that
[docs/design/continuous-quality-tracking.md](docs/design/continuous-quality-tracking.md) designs a
trend-and-regression framework around.

See [docs/upstream-benchmark.md](docs/upstream-benchmark.md) for the two host-specific preparations, what
the suite does **not** require (isolation is only "a one-shot container"), and two real defects the
exercise exposed in our own code: the migrate profile needs a host row that exists on dsh ≥ 0.1.2-alpha.2
but not on the version our image pins, and `migrate-runner.js` is incompatible with that newer dsh.
